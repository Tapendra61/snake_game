...
pub struct FoodGenerator {
	pub food_container: Option<Food>,
	food_exists: bool,
}
...
impl FoodGenerator {
	...
	pub fn generate_food (&mut self, grid: &Grid, snake_handler: &Snake) {
		if !self.food_exists {
			let mut food = Food { pos: grid.generate_rand_coordinate(), color: Color::new(0.4, 0.0, 0.0, 1.0)};
	...
	pub fn destroy_food(&mut self) {
		self.food_container = None;
